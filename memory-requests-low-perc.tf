...
module "memory_requests_low_perc" {
  ...
  query            = "max(${var.cpu_requests_low_perc_evaluation_period}):( max:kubernetes_state.container.memory_requested{${local.cpu_requests_low_perc_filter}} / max:kubernetes_state.node.memory_allocatable{${local.cpu_requests_low_perc_filter}} ) * 100 > ${var.cpu_requests_low_perc_critical}"
  alert_message    = "Kubernetes cluster memory room for Requests in percentage is too low"
  recovery_message = "Kubernetes cluster memory Requests in percentage is ok again."
  ...
  severity           = var.memory_requests_low_perc_severity
