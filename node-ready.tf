...
module "node_ready" {
  ...
  query            = "avg(${var.node_ready_evaluation_period}):max:kubernetes_state.nodes.by_condition{${local.node_ready_filter} AND condition:ready AND (status:true OR status:unknown)} by {host} > ${var.node_ready_critical}"
  alert_message    = "Kubernetes cluster node {{node}} is not ready."
  recovery_message = "Kubernetes cluster node {{node}} is ready again."
